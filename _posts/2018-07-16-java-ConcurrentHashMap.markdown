...
segmentShift表示并发度从高位到最高位1的0的个数，segmentMask表示剩余的位全为1的数，这样只要和segmentMask做与运算就可以定位到分段锁数组的下标位置。因为分段锁数组大小最大16位(65535)，所以segmentShift最大也是65535。
* segmentMask
...
```
上述代码是ConcurrentHashMap的构造函数。这里进行初始化。这里有几个参数，**segmentShift**表示32位的并行度concurrencyLevel的高位0的个数；**segmentMask**表示concurrencyLevel的掩码，也就是剩余的位每一位都是1；这两个参数主要是用来定位Segment数组小标的。
这里有个中间变量c，表示的是每个Segment的HashEntry数组的长度，用initialCapacity/ssize获得，如果除不尽，那么c+1，接下来取大于等于c的最小2次幂值作为HashEntry数组的长度。
最后是创建Segment数组，创建第一个Segment，内部是容量为上边计算出来的HashEntry长度的数组。注意：对于Segment的创建采用的是延迟加载的方式，初始只创建第一个Segment。
...
```
上面是构造函数，**初始化时，sizeCtl设置为1.5*initialCapacity+1**，然后对计算的结果取最近的2的n次方。
